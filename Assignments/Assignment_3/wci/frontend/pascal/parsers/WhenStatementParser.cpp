...
void WhenStatementParser::initialize()
{
    ...
    WHEN_SET.insert(PascalTokenType::NEW_SPECIAL_SYMBOL);
...
ICodeNode *WhenStatementParser::parse_statement(Token *token) throw (string)
{
    ...
    if (token->get_type() == (TokenType) PT_EQU_EQU_GREATER)
    {
        token = next_token(token);  // consume the ==>
    }
    else {
        error_handler.flag(token, MISSING_NEW_SPECIAL_SYMBOL, this);
    }
    return test_node;
}
